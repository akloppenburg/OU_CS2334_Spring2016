...
public class NoozFileProcessorTest {
	...
	public void testReadNoozFileIOExceptionThrown() {
		try {
			NoozFileProcessor.readNoozFile("Bogus Filename");
		} catch (IOException e) {
			return;
		}
	...
	public void testDecodeSource() {
		...
		String returnedSourceName = "";

		Class[] argClasses = { String.class };
		Object[] argObjects = { "1" };
		Method method;
		try {
			method = NoozFileProcessor.class.getDeclaredMethod("decodeSource", argClasses);
			System.err.println(method);
			method.setAccessible(true);
			returnedSourceName = (String) method.invoke(null, argObjects);
		} catch (Exception e) {
			fail("Exception: " + e);
		}

		if ("New York Times".equals(returnedSourceName)) {
			return;
		} else {
			fail("Not equal");
		}
